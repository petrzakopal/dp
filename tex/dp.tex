...
\end{lstlisting}
	...
	\end{figure}
		...
		\subsection{CPU/FPGA Model}\label{subsec:cpu-fpga-model}
			...
			První přístup spočíval ke kontrole výsledků v programu Wolfram Mathematica. Ovšem při kroku simulace $1 \mu$s a simulovaném čase 1 s se jednalo o 1 milion dat. Vykreslení takového množství hodnot, které byly přímkově spojeny, bylo příliš náročné pro zařízení autora a trvalo přiliš dlouho dobu. A protože Wolfram Mathematika není Open Source, bylo od tohoto způsobu upuštěno. Další možností bylo využití programu MATLAB, ovšem opět není Open Source a autor dbal na co největší otevřenosti projektu. Proto bylo nakonec využito jazyku Python, jež není zatížen problémem \textit{Vendor lock-in} a vykreslení požadovaného grafu se stejnou kvalitou jako v MATLABu nebo ve Wolfram Mathematika trvalo velmi krátkou dobu.
