...
source /tools/Xilinx/Vitis/2022.2/settings64.sh\end{lstlisting}
		...
		Aby bylo možné build proces vyvolávat z příkazové řádky, je nutné vytvořit \texttt{makefile} soubory, které slouží ke kompilaci dílčích zdrojových souborů a k vyvolání \texttt{v++} linkeru, jež spojí vytvořené artefakty pro PL do jednoho binárního souboru \texttt{xclbin}. Makefile soubory je možné vytvořit manuálně, nebo pomocí Vitis IDE v okně \textit{Assistant} poklikem levým tlačítkem myši na název potřebného typu procesu build a volby \textit{Create Makefiles}. Toto je potřeba provést nejméně 4x. Pro složky:
		\begin{enumerate}
		...
		Doporučený postup build pro \textit{Hardware} build je následující:
		\begin{enumerate}
	...
	\begin{itemize}
		...
		\item \texttt{<application-name>\_system\_hw\_link/Hardware/<binary-container>.xclbin}
	\end{itemize}
	% vyřešit mezery mezi původním odstavcem a listem a listem a odstavcem
	z build procesu pomocí Vitis také do složky \texttt{transfer}.\par
	% Info o shell.json
	Aby bylo možné využívat příkazy \texttt{xmutil}, které využívají \texttt{DFX-MGR} (daemon) \cite{xilinx-github-dfx-mgr} pro konfiguraci PL, loading a unloading bitstreamů aplikací, spravování data modelu daných bitstreamů apod. je třeba pro akcelerovanou aplikaci vytvořit metadata soubor \texttt{shell.json}. Tento soubor je využíván právě \texttt{DFX-MGR}. Podpora dalších formátů \texttt{shell\_type}, než jen \texttt{XRT\_FLAT} je ve vývoji. \cite{xilinx-github-vitis-tutorials-step-2-create-the-software-components}\par Tento soubor je opět vhodné vytvořit na vývojářském PC a přemístit do složky \texttt{transfer}.\par
