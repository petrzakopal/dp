...
V této sekci budou představeny výsledky a poznatky ohledně spouštěných aplikací, které byly získány analýzou výstupů profilování aplikace pomocí knihovny \texttt{xrt\_coreutil} a programu \textit{Vitis Analyzer}.\par
	...
	\subsection{Preloaded Data – Legacy Application}
		...
		Struktura systému aplikace, která je vizualizována pomocí Vitis Analyzer je zobrazena na \ref{fig:legacy-rt-step-0.000001-system-diagram}.\par
		Na téže snímku je uvedena také informace o skutečném využití zdrojů kernelem. Pro porovnání je využití jednotlivých zdrojů pro aplikace, interagující s kernelem, uvedeno v tab. \ref{tab:vyuziti-zdroju-pl-pro-akcelerovane-aplikace}.

		\begin{figure}[htbp!]
			\centering
			\includegraphics[width=1\textwidth]{src/png/vitis-analyzer/legacy-rt/1M-data/writing-data-output/legacy-rt-step-0.000001-system-diagram.png}
			\caption{System diagram – Vitis Analyzer pro Preloaded Data Application.}
			\label{fig:legacy-rt-step-0.000001-system-diagram}
		\end{figure}



		Tuto aplikace je převážně vhodné analyzovat z pohledu rychlosti zpracování dat v kernelu v závislosti na velikosti přenášených dat.
