...
columns = ["time", "clk"]
df = pd.read_csv("k26-spi-clk-10-mhz-data-0x10000001-clk-detail-mid-copy.csv", names=columns, header=None,)
maxTime = df.time.max()
minTime = df.time.min()
...
# first plot
ax = plt.subplot(111)
ax.set_title("SPI CLK signál 10 MHz – detail", fontsize=30, fontweight=700)
...
plt.plot(df.time, df.clk, color=ctuBlue, label="CLK")

# setting ticks based on min and max time (on x axis)
plt.xticks(np.arange(minTime, maxTime, 0.00000002))
...
# #ticks
# plt.xticks(np.arange(minTime, maxTime, 0.0000002))
...
# save as a file
plt.savefig("pdf/clk-mid-graph.pdf")
