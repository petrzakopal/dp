...
- po náhodném počtu iterrací se zasekne vedlejší thread a jede jen hlavní, když se appka vypne a pustí znova, tak už nejde SPI, inicializuje se, ale nejde přesun dat, ani po xmutil unloadapp nejde..., asi se zasekne na pollingu v spi interruptu někde, musím to asi vyzkoušet s pomocí výpisu dat...

- sh skript pro spuštění aplikace

```sh
xmutil unloadapp&&./download-files-from-server.sh -p kria-1.2&&mv binary_container_1.xclbin binary_container_1.bin &&cp kria-1-2-rt-dp-app binary_container_1.bin pl.dtbo shell.json /lib/firmware/xilinx/rt/&&xmutil loadapp rt&&./kria-1-2-rt-dp-app binary_container_1.bin

```

- funkční kód bez kernelu (byl jen na Vadd) ale funkční SPI a spi v timeru
